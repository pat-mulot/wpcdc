<?php
get_header();
// include __DIR__ . '/includes/home-data.php';
...
$registerForm = $args["registerForm"];
$error = $args["error"];
$error_login = $args["error_login"];
?>
<!-- ------------------------------------------ -->
<main class="main-container">
    <section class="main_section" style="background-image: url(<?= get_theme_file_uri(); ?>/assets/img/background_paper.jpg);">
        <div class="registration-header">
            <a href="#gueritte1">
                <h2 class="main_card-title1">Inscription ?</h2>
                <span>Gueritte numéro 1.</span>
            </a>
            <a href="#gueritte2">
                <h2 class="main_card-title1">Connexion ?</h2>
                <span>Gueritte numéro 2.</span>
            </a>
        </div>
        <hr class="card-separation">
        <?php if ($error) : ?>
            <em class="error_ms"><?= $error; ?></em>
        <?php endif; ?>
        <?php if ($registerForm) : ?>
            <form action="<?= home_url(); ?>/user/add/" method="POST" class="main_card-container">
                <div class="user_form-header">
                    <div>
                        <span id="gueritte1">Gueritte numéro 1 :</span>
                        <h2 class="main_card-title2">Inscription</h2>
                    </div>
                </div>
        ...
        <?php endif; ?>
        <?php if ($error_login) : ?>
            <em class="error_ms">Une erreur est survenue, vérifiez que vous avez correctement remplit les champs.</em>
        <?php endif; ?>
        <form action="<?= site_url('/wp-login.php'); ?>" method="POST" name="loginform" id="loginform" class="main_card-container" autocomplete="off">
            <div class="user_form-header">
                <div>
                    <span id="gueritte2">Gueritte numéro 2 :</span>
                    <h2 class="main_card-title2">Connexion</h2>
                </div>
            </div>
            <div class="main-card-content">
                <div class="user_form-input_container">
